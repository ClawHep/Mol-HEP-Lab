...
export const INITIAL_AGENTS: MolAgent[] = [
  ...
  makeAgent('P3-01', 'Signal Lead',            AgentLayer.EXECUTION,      'run-001'),
  makeAgent('P3-02', 'Background Estimator',   AgentLayer.EXECUTION,      'run-002'),
  makeAgent('P3-03', 'ML Specialist',          AgentLayer.EXECUTION,      'run-003'),
...
// Task descriptions aligned with 18-stage backend model
const TASK_DETAILS: Record<number, string[]> = {
  1:  ['解析研究课题，生成 SMART 目标...', '检测硬件环境 (GPU/CPU)...'],
  2:  ['将课题分解为子问题树...', '确定优先研究方向...'],
  3:  ['规划检索策略，确定数据源...', '生成关键词查询组合...', '调用 OpenAlex API 检索论文...'],
  4:  ['基于相关性和质量筛选文献...', '评估 shortlist 覆盖度... [GATE]'],
  5:  ['从筛选论文提取知识卡片...', '结构化关键发现...'],
  6:  ['聚类研究主题，识别研究空白...', '综合多论文结论...', '生成可证伪假设...'],
  7:  ['设计实验方案 (YAML)...', '确定 baseline 和评估指标... [GATE]', '规划消融实验矩阵...'],
  8:  ['检索相关代码库...', '构建代码上下文...'],
  9:  ['生成实验核心代码...', 'AST 验证代码正确性...', '执行冒烟测试...', 'LLM 修复失败用例...'],
  10: ['估算资源需求...', '在沙箱中执行实验代码...', '编辑-运行-评估循环...', '收敛性评估...'],
  11: ['分析实验指标...', '生成可视化图表...', '多 agent 结果评审...'],
  12: ['做出研究决策: PROCEED / PIVOT / REFINE...', '记录决策历史...'],
  13: ['归纳跨实验知识...', '生成知识摘要...'],
  14: ['生成论文大纲...', '规划各章节结构...'],
  15: ['撰写论文初稿...', '自我修订论文...', '根据评审意见修订...'],
  16: ['物理正确性审查...', '统计方法审查...', '系统不确定性审查...'],
  17: ['整体质量评估...', '图表验证...', '排版审查...'],
  18: ['生成归档清单...', '编译 LaTeX → PDF...', '引用验证...'],
};
...
function stageToRepo(stage: RCStageT): RepoId | null {
  if (stage <= 6) return RepoId.KNOWLEDGE;
  if (stage === 7 || stage === 8) return RepoId.EXP_DESIGN;
  if (stage === 9 || stage === 10) return RepoId.CODEBASE;
  if (stage >= 11 && stage <= 13) return RepoId.RESULTS;
  if (stage >= 14) return RepoId.PAPERS;
  return null;
}
