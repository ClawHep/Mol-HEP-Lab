...
function upsertAgent(agents: MolAgent[], payload: MolAgent): MolAgent[] {
...
export default function App() {
  ...
  const [locale, setLocale] = useState<Locale>(() =>
    (localStorage.getItem('mol-locale') as Locale) || 'en'
  );
  const [theme, setTheme] = useState<'dark' | 'light'>(() =>
    (localStorage.getItem('mol-theme') as 'dark' | 'light') || 'dark'
  );
  useEffect(() => {
  ...
  const localeCtx = useMemo(() => ({
    locale, setLocale: (l: Locale) => { setLocale(l); localStorage.setItem('mol-locale', l); }, t,
  }), [locale, t]);
